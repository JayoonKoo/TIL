...
클래스 외부에서 접근하는 것을 막기 위한 접근 제언자를 타입스크립트에서는 지원한다.
...
Js 에서 private 변수를 표현하기 위해서 * 를 붙혔었는데 TS 에서 관례적으로 private 변수 앞에는 *를 붙힌다. ( 없어도 상관 없음. )

## initialization in constructor param

생성자 안에 접근 제언자를 넣음으로서 생성함과 동시에 할당가지 할수 있다.
